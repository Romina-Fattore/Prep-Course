...
function numberOfCharacters(string) {
  ...
  for ( var i= 0; i<string.length; i++) {
    if (!cuantasVeces.hasOwnProperty (string[i]))
    cuantasVeces [string[i]] = 1;
    else {
      cuantasVeces [string [i]] ++
    }
  }
  return cuantasVeces;
}
...
function asAmirror(str) {
  ...
  //Escribe tu código aquí
  var alReves = str.split (" ");
  for ( var i= 0; i< alReves.length; i++) {
    alReves[i] = alReves [i].split ("").reverse().join("");
  }
  return alReves.join (" ");
} 
...
function deleteAbc(cadena){
  ...
  for ( var i= 0; i< cadena.length; i++) {
    if(cadena[i]==="a"|| cadena[i]==="b"||cadena[i]==="c"){
      ...
      eliminaAbc = eliminaAbc+cadena[i];
    }
  }
  return eliminaAbc;
}
